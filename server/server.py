...
def log(message, client=None):
  ...
  if client is None:
    print('\033[92m[%s]\033[0m %s' %
          (time.strftime(r'%H:%M:%S, %m.%d.%Y'), message))
  else:
    print('\033[92m[%s] %s:%d\033[0m %s' %
          (time.strftime(r'%H:%M:%S, %m.%d.%Y'),
           client[0], client[1], message))


class DataSockListener(threading.Thread):
  ''' Asynchronously accepts data connections '''
  def __init__(self, server):
    super(DataSockListener, self).__init__()
  ...
  def run(self):
    ...
    while True:
      try:
        ...
        if self.server.dataSock is not None:  # Existing data connection not closed, cannot accept
          dataSock.close()
...
class FTPServer(threading.Thread):
  ...
  def __init__(self, ctrSocket, cliAddre):
    super(FTPServer, self).__init__()
    self.daemon = True  # Daemon
    self.bufSize = 1024
    self.controlSock = ctrSocket
    self.clientAddr = cliAddre
    ...
    while True:
      ...
      if cmdHead == 'QUIT':  # QUIT
        ...
        if not self.authenticated:
          ...
          if self.dataListenSock is not None:  # Close existing data connection listening socket
        ...
        if not self.authenticated:
          ...
          self.dataSock.send(directory.encode('utf8'))
          self.dataSock.close()
          self.dataSock = None
...
if __name__ == '__main__':
  ...
  log('Server started.')
  while True:
    (controlSock, client_addr) = listenSock.accept()
    FTPServer(controlSock, client_addr).start()
    log("Connection accepted.", client_addr)
